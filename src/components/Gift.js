...
const Gift = ({id, title, description, user, showManageActions = false}) => {
  const actionBar = showManageActions
    ? <div><Link className="text-indigo no-underline font-bold text-sm" to={`/gifts/${id}/edit`}>Edit</Link><Link className="text-red no-underline font-bold text-sm ml-4" to={`/gifts/${id}/remove`}>Remove</Link></div>
